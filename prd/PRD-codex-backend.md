...
- **Q5 — Model selection**: which Codex model is the default for Ortus projects, and how is it surfaced (a `codex_model` copier question, or config-only)? **Resolved (Phase 3, ortus-z5tj): config-only, and the default is deliberately unpinned.**

  `.codex/config.toml` carries a documented, commented-out `model` key; leaving it unset means the Codex CLI's own current default applies. Pinning a name in the template would go stale the moment OpenAI moves that default, and every project generated before the move would carry the old name forever with no signal that it had aged. Projects that need a specific model (reproducibility, cost, a capability only one model has) uncomment one line in a file they already own.

  No `codex_model` copier question: a question bakes the answer in at generation time, so re-answering means a `copier update`, whereas the config edit is a one-line commit in the project that owns the decision — and the question would add a second place for the same setting to live (NFR-002). `ORTUS_CODEX_MODEL` stays as the per-run override, appended as `-m <name>` by `backend.sh` and resolved by the CLI ahead of the config value. Precedence: `ORTUS_CODEX_MODEL` > `.codex/config.toml` > CLI default.

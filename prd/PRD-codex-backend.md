...
- **Q2 — Exact Codex `--json` event schema fields** (payload shape of `item.*` for command execution, assistant text, and file edits) needed to render `tail.sh` at parity. Pin against a live `codex exec --json` run.

<!--
SUPERSEDED 2026-05-17 (ortus-ylv1).

This file is the bash-era predecessor of `src/ortus/prompts/grind-prompt.md`.
The Python orchestrator (`ortus grind`) loads grind-prompt.md via
`src/ortus/core/prompts.py` (FR-025) — that file is the active source of truth
for per-task instructions and carries every section here plus the /goal
evaluator adaptations (no `<promise>COMPLETE|EMPTY</promise>` sentinels; the
outer `bd ready` poll judges queue exhaustion).

Do NOT edit this file directly. Edit `src/ortus/prompts/grind-prompt.md`
instead. This file is retained until Phase 5 sunset (ortus-sod1.3) deletes
the bash sources; until then `tests/test_grind_prompt_content.py` enforces
that the structural markers shared by both files stay present in
grind-prompt.md.
-->
